...
// Function to insert a node into a binary tree-like structure
TreeNode* insertTreeNode(TreeNode *root, PokemonNode *pokemonNode) {
    if (root == NULL) {
        root = (TreeNode *)malloc(sizeof(TreeNode));
        root->data = pokemonNode;
        root->left = root->right = NULL;
    } else if (pokemonNode->data->id < root->data->data->id) {
        root->left = insertTreeNode(root->left, pokemonNode);
    } else {
        root->right = insertTreeNode(root->right, pokemonNode);
    }
    return root;
}

// Function to display the tree in BFS (Level-Order)
void bfs(TreeNode *root) {
    if (root == NULL) {
        printf("No Pokémon found.\n");
        return;
    }

    // Initialize a queue for BFS
    TreeNode *queue[100];  // Array to hold nodes
    int front = 0, rear = 0;
    queue[rear++] = root;  // Add the root to the queue

    while (front < rear) {
        TreeNode *current = queue[front++];  // Dequeue the first element
        printf("ID: %d, Name: %s\n", current->data->data->id, current->data->data->name);
...
void preorder(TreeNode *root) {
    if (root != NULL) {
        printf("ID: %d, Name: %s\n", root->data->data->id, root->data->data->name);
        preorder(root->left);
        preorder(root->right);
    }
}
...
// Function to display Pokemon alphabetically by name
void alphabetical(PokemonNode *root) {
    PokemonNode *pokemonArray[100];  // Array to hold all Pokemon nodes
    int count = 0;
    
    // Collect all nodes into an array
    PokemonNode *current = root;
    while (current != NULL) {
        pokemonArray[count++] = current;
        current = current->next;
    }

    // Sort the array alphabetically by name
    qsort(pokemonArray, count, sizeof(PokemonNode *), compareByName);

    // Print the sorted list
    for (int i = 0; i < count; i++) {
        printf("ID: %d, Name: %s\n", pokemonArray[i]->data->id, pokemonArray[i]->data->name);
    }
}
...
// Function to convert linked list to binary tree and perform traversal
void performTraversals(PokemonNode *root,int choice) {
    TreeNode *treeRoot = NULL;
    
    // Convert linked list to binary tree
    PokemonNode *current = root;
    while (current != NULL) {
        treeRoot = insertTreeNode(treeRoot, current);
        current = current->next;
    }
...
// Function to free the binary tree nodes
...
void freePokemonNode(PokemonNode *node) {
    if (node != NULL) {
        // First free the Pokemon data
        free(node->data); // Assuming PokemonData was dynamically allocated, free it
        // Then free the PokemonNode itself
        free(node);
    }
}

// Function to free an OwnerNode
void freeOwnerNode(OwnerNode *owner) {
    if (owner != NULL) {
        // First, free the pokedex linked list
        ...
        // Then, free the owner name if it was dynamically allocated (if applicable)
        // (In your case, it looks like a static array, so no need to free `ownerName`)

        // Finally, free the OwnerNode itself
        free(owner);
    }
}
