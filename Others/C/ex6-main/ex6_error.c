...
TreeNode* createTreeNode(PokemonData *data) {
...
// Function to insert a node into the binary tree based on the Pokemon id
TreeNode* insertTreeNode(TreeNode *root, PokemonData *data) {
    // If the tree is empty, create a new node
    if (root == NULL) {
        return createTreeNode(data);
    }

    // If the data's id is less than the current node's id, insert into the left subtree
    if (data->id < root->data->id) {
        root->left = insertTreeNode(root->left, data);
    }
    // If the data's id is greater than the current node's id, insert into the right subtree
...
// Function to convert the linked list to a binary search tree based on the id
TreeNode* createBinaryTree(PokemonNode *root) {
    TreeNode *treeRoot = NULL;
    
    // Traverse the linked list and insert each PokemonNode's data into the binary tree
    PokemonNode *current = root;
    while (current != NULL) {
        treeRoot = insertTreeNode(treeRoot, current->data);
...
// Function to create a new QueueNode for BFS
...
void levelOrderTraversal(TreeNode *root) {
    ...
    QueueNode *node = createQueueNode(root);
    front = rear = node;

    while (front != NULL) {
        TreeNode *currentNode = front->treeNode;
        printf("ID: %d, Name: %s\n", currentNode->data->id, currentNode->data->name);

        // Enqueue left child
        if (currentNode->left != NULL) {
            rear->next = createQueueNode(currentNode->left);
            rear = rear->next;
        }

        // Enqueue right child
        if (currentNode->right != NULL) {
            rear->next = createQueueNode(currentNode->right);
            rear = rear->next;
        }
...
// Helper function to collect all nodes in an array for sorting by name
void collectNodesByName(TreeNode *root, TreeNode **nodes, int *index) {
    if (root != NULL) {
        collectNodesByName(root->left, nodes, index);
        nodes[*index] = root;
        (*index)++;
        collectNodesByName(root->right, nodes, index);
    }
}
void freeTree(TreeNode *root) {
    if (root != NULL) {
        ...
        free(root->data);  // Only free if data was dynamically allocated
        
        // Free the TreeNode structure itself
        free(root);
    }
}

// Alphabetical (by name) Traversal
void alphabeticalTraversal(TreeNode *root) {
    if (root == NULL) {
        printf("Tree is empty.\n");
        return;
    }
    ...
    int size = 0;
    TreeNode *nodes[100]; // Assuming maximum 100 nodes in the tree
    int index = 0;

    // Collect nodes in in-order sequence
    collectNodesByName(root, nodes, &index);

    // Sort nodes by name using a simple bubble sort
    for (int i = 0; i < index - 1; i++) {
        for (int j = i + 1; j < index; j++) {
            if (strcmp(nodes[i]->data->name, nodes[j]->data->name) > 0) {
    ...
    // Print nodes in alphabetical order
    for (int i = 0; i < index; i++) {
        printf("ID: %d, Name: %s\n", nodes[i]->data->id, nodes[i]->data->name);
    }
}
// Display the Pokedex
void displayPokedex(PokemonNode *root) {
    ...
    TreeNode *binaryTreeRoot = createBinaryTree(root);
    switch(choice){
    case 1:
    // printf("\nLevel-Order (BFS):\n");
    levelOrderTraversal(binaryTreeRoot);
    break;
    case 2:
    // printf("\nPre-Order:\n");
    ...
    alphabeticalTraversal(binaryTreeRoot);
    break;
    }
