...
This document explains how John (the receiver) can decrypt and verify the message sent by Debmalya (the sender) using the provided OpenSSL-based RSA-KEM + AES-256 scheme.
...
| `debmalya_hazra.txt.enc` | Ciphertext — the AES-256-CBC encrypted message.                                              | Public           |
| `signature.bin`        | Digital signature over the ciphertext, created using sender’s private key.                   | Public           |
| `debmalya_public.pem`    | Sender’s public RSA key (used to verify the digital signature).                              | Public           |
...
openssl enc -d -aes-256-cbc -in debmalya_hazra.txt.enc -out debmalya_hazra.txt -K $KEYHEX -iv $IVHEX
openssl dgst -sha1 -verify debmalya_public.pem -signature signature.bin debmalya_hazra.txt.enc
```
